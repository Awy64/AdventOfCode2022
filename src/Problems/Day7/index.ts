...
const requiredByUpdate = 30000000;
...
const input = fs.readFileSync('input.txt', 'utf8').split('$').filter(emptyLine);
...
const totalSizeUsed = finalFileSystem.folders['/'].size;
const unusedSpace = totalDiskSpace - totalSizeUsed;
const spaceNeeded = requiredByUpdate - unusedSpace;
const allFolderSizesOrdered = Object.values(finalFileSystem.folders).map(folder => folder.size).sort((a,b) => a-b);
const sizesBiggerThanNeeded = allFolderSizesOrdered.filter(size => size >= spaceNeeded);

console.log(sizesBiggerThanNeeded[0]);
